...
const plans: Array<{ id: PlanId; label: string; price: string; period: string; detail: string; featured?: boolean }> = [
  ...
  { id: "yearly", label: "Yearly Lifetime", price: "$59", period: "/ year", detail: "Lifetime-equivalent support for mental wellness." },
];
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (error instanceof TypeError && error.message === "Failed to fetch") {
        setMessage("We could not reach the secure payment server. If the server is spinning up, this can take up to 40 seconds. Please try again.");
      } else {
    ...
    <section id="premium" className="scroll-mt-24 border-b border-stone-200 py-10 sm:py-16">
      <div className="relative overflow-hidden rounded-[1.5rem] bg-[#18374A] px-5 py-8 text-white shadow-[0_22px_60px_rgba(22,55,74,.22)] sm:rounded-[2rem] sm:px-8 sm:py-10 lg:px-10">
        ...
        <div className="relative grid gap-8 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)] lg:items-start">
          <div>
            <div className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-2.5 py-1 text-[10px] font-bold tracking-[.14em] text-[#FBE8B6] uppercase sm:gap-2 sm:px-3 sm:py-1.5 sm:text-[11px]"><Crown className="size-3 sm:size-3.5" /> Clario Premium</div>
            <h2 className="mt-4 max-w-xl font-display text-3xl leading-[1.05] tracking-tight sm:mt-5 sm:text-5xl sm:leading-[.98]">Make more room for emotional presence.</h2>
            <p className="mt-3 max-w-xl text-sm leading-6 text-white/76 sm:mt-4 sm:text-[15px] sm:leading-7">Choose a plan to pre-book Clario Premium. Standard Clario is completely free and private, but Premium pre-booking unlocks deeper visual statistics and supports independent, ad-free mental health software development.</p>

            <div className="mt-6 flex flex-col gap-2.5 sm:mt-7 sm:grid sm:grid-cols-3 sm:gap-3">
              {plans.map((item) => {
                const selected = plan === item.id;
                return <button key={item.id} type="button" onClick={() => setPlan(item.id)} aria-label={`Select Clario Premium ${item.label} pricing plan`} className={`relative flex w-full items-center justify-between rounded-xl border px-4 py-3.5 text-left transition duration-200 sm:block sm:rounded-2xl sm:p-4 ${selected ? "border-[#F9DEA3] bg-white/16 shadow-[0_10px_24px_rgba(0,0,0,.15)]" : "border-white/14 bg-white/[.06] hover:border-white/30 hover:bg-white/[.1]"}`} aria-pressed={selected}>
            ...
            <div className="mt-5 flex items-center gap-1.5 text-[11px] leading-4 text-white/66 sm:mt-7 sm:gap-2 sm:text-xs sm:leading-5"><ShieldCheck className="size-3.5 shrink-0 text-[#A4D7C6] sm:size-4" /> Your data is encrypted and kept entirely private within your personal profile.</div>
          </div>
          ...
          <form onSubmit={submit} className="rounded-[1.25rem] border border-white/16 bg-[#0E2635]/72 p-5 shadow-xl backdrop-blur-sm sm:rounded-[1.45rem] sm:p-6">
            <p className="text-[11px] font-bold tracking-[.15em] text-[#A4D7C6] uppercase sm:text-xs">Pre-book Clario Premium</p>
            ...
            <>
              ...
              <button type="submit" disabled={submitting} aria-label={`Submit pre-booking form and pre-book Clario Premium for ${plans.find((item) => item.id === plan)?.price}`} className="mt-4 inline-flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-[#F9DEA3] px-4 text-sm font-extrabold text-[#34513E] transition hover:bg-[#FFF0C8] disabled:cursor-wait disabled:opacity-70 sm:mt-5 sm:h-11">
            ...
            <p className="mt-3 text-center text-[11px] leading-5 text-white/52">You will be redirected to secure checkout to complete this pre-booking.</p>
          </form>
        </div>
      </div>
    </section>
